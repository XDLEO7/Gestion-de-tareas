...
if (isset($_GET['id'])) {
    ...
    if ($result->num_rows == 1) {
        ...
        $id_prioridad = $row['id_prioridad']; // Añadido para la prioridad
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $id_prioridad = $_POST['id_prioridad']; // Recibir prioridad del formulario
...
// Obtener prioridades para el select
...
<head>
    ...
    <link rel="stylesheet" href="css/actualizar.css"> <!-- Enlace al archivo CSS externo -->
</head>
<body>
    ...
    <form method="POST">
        ...
        <select id="prioridad" name="id_prioridad" required>
            <?php
            if ($result_prioridades->num_rows > 0) {
                while($prior = $result_prioridades->fetch_assoc()) {
                    echo "<option value='" . $prior['id_prioridad'] . "'" . ($prior['id_prioridad'] == $id_prioridad ? " selected" : "") . ">" . $prior['nivel_prioridad'] . "</option>";
                }
            }
            ?>
        </select>
